var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
/**加密模块
 * 1.0 数据加密
 * 2.0 数据解密
 * 3.0 数据映射
 */
var network;
(function (network) {
    //数据类型
    network.DK_MAPPED = 0x01; //映射类型
    network.DK_ENCRYPT = 0x02; //加密类型
    network.VER_SOCKET = 1; //网络版本
    var g_dwPacketKey = 0xA55AA55A;
    //映射字符
    var m_cbSendByteMap = [
        
    ];
    //映射字符
    var m_cbRecvByteMap = [
        
    ];
    var Crypt = (function () {
        /**
         * 构造
         */
        function Crypt() {
            /**
             * 发送密钥
             */
            this.m_dwSendXorKey = 0x12345678;
            /**
             *接收密钥
             */
            this.m_dwRecvXorKey = 0x12345678;
            /**
             * 发送计数
             */
            this.m_dwSendPacketCount = 0;
            /**
             * 接收计数
             */
            this.m_dwRecvPacketCount = 0;
            /**
             * 字节映射
             */
            this.m_cbSendRound = 0;
            this.m_cbRecvRound = 0;
            /**
             * 加密标志 默认DK_MAPPED
             */
            this.m_cbDataFlag = network.DK_MAPPED;
        }
        /**
         * 设置加密标识
         */
        Crypt.prototype.setDataFlag = function (cbFlag) {
            this.m_cbDataFlag = cbFlag;
        };
        /**
         * 获取加密标识
         */
        Crypt.prototype.getDataFlag = function () {
            return this.m_cbDataFlag;
        };
        // 生成随机数
        Crypt.prototype.SeedRandMap = function (wSeed) {
            var dwHold = utils.getUnsignedDWORD(wSeed);
            return utils.getUnsignedWORD((dwHold = dwHold * 241103 + 2533101) >> 16);
        };
        Crypt.prototype.MapSendByte = function (byte, cbSendRound) {
            var index = utils.getUnsignedByte(byte + utils.getUnsignedByte(cbSendRound));
            var cbMap = utils.getUnsignedByte(m_cbSendByteMap[index]);
            return cbMap;
        };
        Crypt.prototype.MapRecvByte = function (byte, cbRecvRound) {
            var cbMap = m_cbRecvByteMap[utils.getUnsignedByte(byte)] - cbRecvRound;
            return cbMap = utils.getUnsignedByte(cbMap);
        };
        //加密数据
        Crypt.prototype.EncryptBuffer = function (buffer, wSize) {
            // 变量定义
            var wDataSize = wSize;
            var cbCheckCode = 0;
            do {
                //偏移前置
                buffer.position(0);
                // 调整长度(DWORD的整数倍)
                var wEncryptSize = wDataSize - df.Len_Tcp_Info;
                var wSnapCount = 0;
                if (wEncryptSize % 4 != 0) {
                    wSnapCount = 4 - wEncryptSize % 4;
                    utils.Memory.memset(buffer, 0, wSnapCount, df.Len_Tcp_Info + wEncryptSize);
                }
                // 映射字节
                for (var i = df.Len_Tcp_Info; i < wDataSize; i++) {
                    //校验字段
                    cbCheckCode += buffer.getByteArray().bytes[i];
                    //防止字节溢出
                    cbCheckCode = utils.getUnsignedByte(cbCheckCode);
                    //字节操作
                    buffer.getByteArray().bytes[i] = this.MapSendByte(buffer.getByteArray().bytes[i], this.m_cbSendRound);
                    this.m_cbSendRound += 3;
                    this.m_cbSendRound = utils.getUnsignedByte(this.m_cbSendRound);
                }
                cbCheckCode = ~cbCheckCode + 1;
                cbCheckCode = utils.getUnsignedByte(cbCheckCode);
                // 创建密钥
                var dwXorKey = this.m_dwSendXorKey;
                if (this.m_dwSendPacketCount == 0) {
                    dwXorKey = Math.floor(Math.random() * 0xFFFF);
                    // 映射种子
                    dwXorKey = this.SeedRandMap(utils.getUnsignedWORD(dwXorKey));
                    var value = this.SeedRandMap(utils.getUnsignedWORD(dwXorKey >> 16));
                    dwXorKey |= utils.getUnsignedDWORD(value) << 16;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    dwXorKey ^= g_dwPacketKey;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    this.m_dwSendXorKey = dwXorKey;
                    this.m_dwRecvXorKey = dwXorKey;
                }
                //记录偏移
                buffer.position(df.Len_Tcp_Info);
                var offset0 = df.Len_Tcp_Info;
                //变量定义
                var pdwXor = 0;
                var pwSeed = 0;
                var wEncryptCount = (wEncryptSize + wSnapCount) / 4;
                //加密数据
                for (var i = 0; i < wEncryptCount; i++) {
                    buffer.position(offset0);
                    pdwXor = buffer.Pop_DWORD();
                    offset0 += 4;
                    //字节操作
                    pdwXor ^= dwXorKey;
                    pdwXor = utils.getUnsignedDWORD(pdwXor);
                    //加密字节
                    var temp = new utils.ByteArray();
                    temp.Append_INT(pdwXor);
                    utils.Memory.CopyMemory(buffer, temp, 4, offset0 - 4, 0);
                    temp.position(0);
                    pwSeed = temp.Pop_WORD();
                    dwXorKey = this.SeedRandMap(pwSeed);
                    pwSeed = temp.Pop_WORD();
                    dwXorKey |= (this.SeedRandMap(pwSeed)) << 16;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    dwXorKey ^= g_dwPacketKey;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    //console.log(`dwXorKey ========================${dwXorKey}`);
                }
                //插入密钥
                if (this.m_dwSendPacketCount == 0) {
                    //包头内容
                    var temp_1 = new utils.ByteArray();
                    utils.Memory.CopyMemory(temp_1, buffer, df.Len_Tcp_Head, 0, 0);
                    //插入秘钥
                    var temp1 = new utils.ByteArray();
                    temp1.Append_DWORD(this.m_dwSendXorKey);
                    utils.Memory.CopyMemory(temp1, buffer, wSize - df.Len_Tcp_Head, 4, df.Len_Tcp_Head);
                    //组合数据
                    buffer.clear();
                    utils.Memory.CopyMemory(buffer, temp_1, df.Len_Tcp_Head, 0, 0);
                    utils.Memory.CopyMemory(buffer, temp1, temp1.getLength(), df.Len_Tcp_Head, 0);
                    //引用置空
                    temp_1 = null;
                    temp1 = null;
                    //组合长度
                    wDataSize += 4;
                }
                //设置变量
                this.m_dwSendPacketCount++;
                this.m_dwSendXorKey = dwXorKey;
            } while (false);
            return [network.VER_SOCKET, cbCheckCode, wDataSize];
        };
        //解密数据
        Crypt.prototype.CrevasseBuffer = function (buffer, wSize, offset) {
            if (offset === void 0) { offset = 0; }
            // 变量定义
            var wDataSize = wSize;
            do {
                //调整长度(DWORD的整数倍)
                var wSnapCount = 0;
                if (wDataSize % 4 != 0) {
                    wSnapCount = 4 - wDataSize % 4;
                    utils.Memory.memset(buffer, 0, wSnapCount, wDataSize);
                }
                //记录偏移
                var offset0 = df.Len_Tcp_Info;
                var offset1 = df.Len_Tcp_Info;
                //变量定义
                var pdwXor = 0;
                var pwSeed = 0;
                var dwXorKey = this.m_dwRecvXorKey;
                var wEncryptCount = (wDataSize + wSnapCount - 4) / 4;
                //解密数据
                for (var i = 0; i < wEncryptCount; i++) {
                    if ((i == (wEncryptCount - 1)) && (wSnapCount > 0)) {
                        var bytes = new utils.ByteArray();
                        bytes.Append_DWORD(this.m_dwRecvXorKey);
                        bytes.position(0);
                        var offset = 4 - wSnapCount;
                        utils.Memory.CopyMemory(buffer, bytes, wSnapCount, wDataSize, offset);
                    }
                    buffer.position(offset1);
                    pwSeed = buffer.Pop_WORD();
                    offset1 += 2;
                    dwXorKey = this.SeedRandMap(utils.getUnsignedWORD(pwSeed));
                    pwSeed = buffer.Pop_WORD();
                    offset1 += 2;
                    dwXorKey |= (this.SeedRandMap(pwSeed)) << 16;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    dwXorKey ^= g_dwPacketKey;
                    dwXorKey = utils.getUnsignedDWORD(dwXorKey);
                    buffer.position(offset0);
                    pdwXor = buffer.Pop_DWORD();
                    offset0 += 4;
                    pdwXor ^= this.m_dwRecvXorKey;
                    pdwXor = utils.getUnsignedDWORD(pdwXor);
                    var temp = new utils.ByteArray();
                    temp.Append_DWORD(pdwXor);
                    utils.Memory.CopyMemory(buffer, temp, 4, offset0 - 4, 0);
                    this.m_dwRecvXorKey = dwXorKey;
                }
                //校验字节
                buffer.position(1);
                var cbCheckCode = utils.getUnsignedByte(buffer.Pop_Byte());
                for (var i = df.Len_Tcp_Info; i < wDataSize; i++) {
                    buffer.getByteArray().bytes[i] = this.MapRecvByte(buffer.getByteArray().bytes[i], this.m_cbRecvRound);
                    this.m_cbRecvRound += 3;
                    this.m_cbRecvRound = utils.getUnsignedByte(this.m_cbRecvRound);
                    cbCheckCode += buffer.getByteArray().bytes[i];
                    cbCheckCode = utils.getUnsignedByte(cbCheckCode);
                }
                egret.warn(cbCheckCode == 0);
                if (cbCheckCode != 0)
                    return false;
            } while (false);
            return true;
        };
        return Crypt;
    }());
    network.Crypt = Crypt;
    __reflect(Crypt.prototype, "network.Crypt");
})(network || (network = {}));
